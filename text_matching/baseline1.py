...
d_k = d_model//n_head
pre_dic = torch.load('./pretrain_model/save_pre_chn_model.pkl')
pre_dic1 = torch.load('./pretrain_model/save_pre_chn_mode1.pkl')
pre_dic2 = torch.load('./pretrain_model/save_pre_chn_mode2.pkl')
...
class DepthwiseSeparableConv(nn.Module):
    def __init__(self, in_ch, out_ch, k, bias=True):
        ...
        if k == 7:
            self.depthwise_conv.weight = nn.Parameter(pre_dic2['conv1.depthwise_conv.weight'])
            self.depthwise_conv.bias = nn.Parameter(pre_dic2['conv1.depthwise_conv.bias'])
            self.pointwise_conv.weight = nn.Parameter(pre_dic2['conv1.pointwise_conv.weight'])
            self.pointwise_conv.bias = nn.Parameter(pre_dic2['conv1.pointwise_conv.bias'])
...
class EncoderBlock(nn.Module):
    ...
    def forward(self, x):
        ...
        # out = self.normb(out)
        x = x.transpose(1,2)
        out1 = self.convs1(x)#(bz*d_model*seq_len)
        out2 = self.convs2(x)#(bz*d_model*seq_len)
        out3 = self.convs3(x)#(bz*d_model*seq_len)
...
#         self.textb_conv = DepthwiseSeparableConv(d_word,d_model,5)
        ...
        self.texta_enc = EncoderBlock(in_ch=d_word,out_ch=d_model,k1=3,k2=5,k3=7)
        self.fc = nn.Sequential(
            nn.BatchNorm1d(36*d_model),
            nn.Linear(36*d_model, 12*d_model),
    ...
    def forward(self,texta,textb):
        ...
        a_enc = self.texta_enc(a_embed)
        b_enc = self.texta_enc(b_embed)

        a_attn, b_attn = soft_attention_align(a_enc, b_enc)
        a_out = torch.cat([a_enc,a_attn,a_enc*a_attn],dim=-1)
        b_out = torch.cat([b_enc,b_attn,b_enc*b_attn],dim=-1)
...
if __name__ == '__main__':
    texta = torch.LongTensor([[1, 2, 4, 3, 2, 1, 6, 7, 4, 5],
                              [2, 0, 3, 0, 2, 0, 1, 0, 0, 7],
                              [5, 20, 42, 0, 0, 0, 6, 7, 0, 9],
                              [9, 30, 0, 0, 2, 0, 0, 0, 4, 0]])
    textb = torch.LongTensor([[2, 0, 0, 1, 1, 0, 0, 1, 1, 0],
                              [20, 0, 1, 6, 1, 3, 18, 0, 0, 0],
                              [51, 1, 1, 0, 1, 4, 1, 0, 7, 0],
                              [34, 0, 0, 1, 1, 4, 0, 0, 1, 2]])
    model = sim_model()
    loss_total = []
    acc_total = []
    for i in range(5):

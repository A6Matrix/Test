...
from DistillModel_Utils/Student_utils import get_mask, replace_masked
from esim_layer import RNNDropout, Seq2SeqEncoder, SoftmaxAttention
from Config import opt

"""
 定义网络时，需要继承 nn.Module，并实现它的 forward 方法。
把网络中具有可学习参数的层放在构造函数__init__中。
不具有学习参数的放在 forward 中使用 nn.functional 代替
 """
device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
class ESIM(nn.Module):
    def __init__(self, embeddings=None, hidden_size=300, dropout=0.2, num_classes=2, device=device):
